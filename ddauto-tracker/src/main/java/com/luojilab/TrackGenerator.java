...
public class TrackGenerator {
    ...
    //埋点配置集合 global_view_id--->TrackConfig
    private Map<String, TrackConfig> mPointConfigs = new ConcurrentHashMap<>();

    private static TrackGenerator sGenerator;
    ...
    public static TrackGenerator getInstance() {
        synchronized (TrackGenerator.class) {
            if (sGenerator == null) {
                sGenerator = new TrackGenerator();
            }
    ...
    public Pair<String, Map<String, Object>> generatePointData(@NonNull String idName, @Nullable Object object) {
        ...
        TrackConfig config = mPointConfigs.get(idName);
        if (config == null) {
    ...
    @NonNull
    private Map<String, Object> getFieldsForConfig(@NonNull TrackConfig config, @Nullable Object data) {

        if (data == null) return new HashMap<>();

        List<TrackConfig.ParamsBean> list = config.getParams();
        //该埋点未配置参数字段，无需上传数据
        if (list == null || list.isEmpty()) return new HashMap<>();

        Map<String, Object> point = new HashMap<>();
        for (TrackConfig.ParamsBean param : list) {
            String p = param.getP();
    ...
    private void cachePointConfigAsMap(@NonNull List<JsonObject> list) {
        ...
        Map<String, TrackConfig> map = new HashMap<>();

        for (JsonObject config : list) {
            if (config == null) continue;

            TrackConfig pc = CoreUtils.json2Bean(config, TrackConfig.class);
            if (pc == null) continue;
        ...
        //解决并发修改问题
        Map<String, TrackConfig> diff = Maps.difference(map, mPointConfigs).entriesOnlyOnRight();
